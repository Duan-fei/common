...
def time_period(start, end):
    ...
    if s_month + s_year == (e_month + e_year):
        if s_month < 10:
            s_month = "0%s" % s_month
        for i in xrange(s_day, e_day + 1):
            if i < 10:
                i = "0%s" % i

            date_list.append("%s-%s-%s" % (s_year, s_month, i))
        ...
        if s_month < 10:
            ...
            month = s_month
        for i in xrange(s_day, s_end + 1):
            if i < 10:
                i = "0%s" % i
            date_list.append("%s-%s-%s" % (s_year, month, i))

        if s_year == e_year:
            for i in xrange(s_month + 1, e_month):
                end = calendar.monthrange(s_year, i)[1]
                if i < 10:
                    i = "0%s" % i
                for b in xrange(1, end + 1):
                    if b < 10:
                        b = "0%s" % b
                    date_list.append("%s-%s-%s" % (s_year, i, b))
        else:
            for i in xrange(s_month + 1, 13):
                end = calendar.monthrange(s_year, i)[1]
                if i < 10:
                    i = "0%s" % i
                for b in xrange(1, end + 1):
        ...
        if e_month < 10:
            ...
            month = e_month
        for i in xrange(1, e_day + 1):
            if i < 10:
                i = "0%s" % i
            date_list.append("%s-%s-%s" % (e_year, month, i))
